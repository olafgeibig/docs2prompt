...
from docs2prompt.arize_phoenix import litellm_instrumentation
litellm_instrumentation()

# Simple single message completion call
litellm.completion(
    model="openrouter/google/gemini-flash-1.5", messages=[{"content": "What's the capital of China?", "role": "user"}]
)
...
# litellm.aembedding(model="text-embedding-ada-002", input=["good morning from litellm"])  # noqa: F704

# # Image generation call
# litellm.image_generation(model="dall-e-2", prompt="cute baby otter")

# # Asynchronous image generation call
# # litellm.aimage_generation(model="dall-e-2", prompt="cute baby otter")  # noqa: F704